...
const categoryMap = {
  math: 19, gaming: 15, games: 15, science: 17, history: 23,
  geography: 22, aviation: 28, vehicles: 28, computers: 18,
  tech: 18, music: 12, movies: 11, sports: 21, anime: 31, general: 9
};
...
async function fetchQuestionsFromWeb(topicInput) {
  ...
  if (topicInput) {
    ...
    if (matchedKey) categoryId = categoryMap[matchedKey];
  }
...
function startTimer(roomCode) {
  ...
  room.timerInterval = setInterval(() => {
    room.timeLeft -= 1;
    io.to(roomCode).emit('timerUpdate', room.timeLeft);
    if (room.timeLeft <= 0) endQuestionRound(roomCode);
  }, 1000);
}
...
io.on('connection', (socket) => {
  ...
  socket.on('submitAnswer', ({ roomCode, answerIndex }) => {
    ...
    if (room.players.every(p => p.answered)) endQuestionRound(roomCode);
  });
